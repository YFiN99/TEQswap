...
// AMAN: Jika tidak ada di .env, kita beri string kosong atau peringatan
export const WC_PROJECT_ID = import.meta.env.VITE_WALLETCONNECT_PROJECT_ID || "";

if (!WC_PROJECT_ID && import.meta.env.MODE === 'development') {
  console.warn("PERINGATAN: VITE_WALLETCONNECT_PROJECT_ID tidak ditemukan di .env!");
}
...
export const TOKENS = [
  ...
  { symbol: "DAI",  name: "DAI",        address: "0xb96a869c74be2ed561d95a77408505371f287d16", decimals: 18, color: "#f7931a", bg: "#1f1000" },
...
export const buildPath = (tIn, tOut) => {
  ...
  if (addrIn === WTEQ_ADDRESS || addrOut === WTEQ_ADDRESS) {
      return [addrIn, addrOut];
  }
  return [addrIn, WTEQ_ADDRESS, addrOut];
};
